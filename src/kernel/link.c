...
//注册link状态变化处理函数
BOOL APIFUNC(sys_register_linkchange)(SYS_LINKCHANGE_FUNC func, void *arg) REENTRANT
{
    uint8 i;
...
//取消注册link变化处理函数
void  APIFUNC(sys_unregister_linkchange)(SYS_LINKCHANGE_FUNC func) REENTRANT
{
    uint8 i;
...
APISTATIC void  APIFUNC(sys_linkchange_timer)(void *arg) REENTRANT
{
    uint16 uport;
    uint8 i;
    BOOL link;
...
/**link状态变化的处理函数**/
void  APIFUNC(sys_linkchange_init)(void) REENTRANT
{
    uint16 i;
    //设置所有端口的link处理函数为空
    for(i=0; i<CFG_MAX_REGISTERED_LINKCHANGE; i++) 
    ...
    //设置所有端口的link状态为没有link
    SAL_UPORT_ITER(i) {
        linkchange_status[i] = FALSE;
    }
    //在计时器中添加link状态变化的处理函数
